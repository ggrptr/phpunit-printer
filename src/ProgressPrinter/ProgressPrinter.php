...
namespace ggrptrr\PHPUnitPrinter\ProgressPrinter;
...
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\BeforeTestClassMethodErroredSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestConsideredRiskySubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestErroredSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestFailedSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestFinishedSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestMarkedIncompleteSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestPreparedSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestRunnerExecutionStartedSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestSkippedSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestSuiteStartedSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredDeprecationSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredNoticeSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredPhpDeprecationSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredPhpNoticeSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredPhpunitDeprecationSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredPhpunitWarningSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredPhpWarningSubscriber;
use ggrptrr\PHPUnitPrinter\ProgressPrinter\Subscriber\TestTriggeredWarningSubscriber;
use function floor;
...
final class ProgressPrinter
{
     ...
     */
    public function __construct(Printer $printer, Facade $facade, Configuration $configuration)
    {
    ...
    public function testRunnerExecutionStarted(ExecutionStarted $event): void
    {
        ...
        $this->column             = 0;
        $this->maxColumn          = $this->configuration->columns() - strlen('  /  (XXX%)') - (2 * $this->numberOfTestsWidth);
    }
    ...
    public function testTriggeredNotice(NoticeTriggered $event): void
    {
        ...
        if ($source->restrictNotices() &&
            !(new SourceFilter)->includes($this->configuration->source(), $event->file())) {
            return;
        }

        if (!$source->ignoreSuppressionOfNotices() && $event->wasSuppressed()) {
            return;
        }
    ...
    public function testTriggeredPhpNotice(PhpNoticeTriggered $event): void
    {
        ...
        if ($source->restrictNotices() &&
            !(new SourceFilter)->includes($source, $event->file())) {
            return;
        }

        if (!$source->ignoreSuppressionOfPhpNotices() && $event->wasSuppressed()) {
            return;
        }
    ...
    public function testTriggeredDeprecation(DeprecationTriggered $event): void
    {
        ...
        if ($source->restrictDeprecations() &&
            !(new SourceFilter)->includes($source, $event->file())) {
            return;
        }

        if (!$source->ignoreSuppressionOfDeprecations() && $event->wasSuppressed()) {
            return;
        }
    ...
    public function testTriggeredPhpDeprecation(PhpDeprecationTriggered $event): void
    {
        ...
        if (!$source->ignoreSuppressionOfPhpDeprecations() && $event->wasSuppressed()) {
            return;
        }
    ...
    public function testTriggeredWarning(WarningTriggered $event): void
    {
        ...
        if ($source->restrictWarnings() &&
            !(new SourceFilter)->includes($source, $event->file())) {
            return;
        }

        if (!$source->ignoreSuppressionOfWarnings() && $event->wasSuppressed()) {
            return;
        }
    ...
    public function testTriggeredPhpWarning(PhpWarningTriggered $event): void
    {
        ...
        if (!$source->ignoreSuppressionOfPhpWarnings() && $event->wasSuppressed()) {
            return;
        }
    ...
    public function testTriggeredError(ErrorTriggered $event): void
    {
        ...
        if (!$source->ignoreSuppressionOfErrors() && $event->wasSuppressed()) {
            return;
        }
    ...
    {
        if (!in_array($event->testSuite()->name(), array_map( function ($item) {
            return $item->name();
