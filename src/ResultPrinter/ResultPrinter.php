...
final class ResultPrinter
{
    ...
    public function applicationFinished(Finished $event): void {
        ...
        $duration = $event->telemetryInfo()->time()->duration($event->telemetryInfo()->time());
        $timer = new Timer();
        $timer->start();
        $duration = $timer->stop();
        ...
        if ($result->numberOfTestsRun() > 0) {
          ...
          //  }
            ...
            $this->printer->print((new ResourceUsageFormatter)->resourceUsage($duration) . PHP_EOL . PHP_EOL);
        }

        // TODO Ezt kitisztítani
        /*
